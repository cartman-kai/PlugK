...
namespace UIManager
{
    ...
    std::string KeyCodeToString(int key)
    {
        ...
        if (key >= '0' && key <= '9') { buf[0] = (char)key; return std::string(buf); }
        if (key >= 'A' && key <= 'Z') { buf[0] = (char)key; return std::string(buf); }
    ...
    void RenderKeyBind(const char *label, int *keyVal)
    {
        ...
        if (btnLabel.empty()) btnLabel = "NONE";
    ...
    void RenderMainPage()
    {
        ...
        if (btnWidth < 300 * g_dpiScale) btnWidth = 300 * g_dpiScale;
        ...
        auto CenteredButton = [&](const char* label, bool enabled, const ImVec4& colorOverride = ImVec4(0,0,0,0)) {
            ...
            if (!enabled) { ImGui::PopStyleColor(); ImGui::PopStyleVar(); }
            else if (colorOverride.w > 0.0f) { ImGui::PopStyleColor(3); }
        ...
        ImGui::SetCursorPosX(cursorX);
        if (!status.exeExists || !status.dllExists) ImGui::PushStyleVar(ImGuiStyleVar_Alpha, 0.5f);
        
        // This uses the default Vibrant Blue we set in SetupStyles
        if (ImGui::Button("启动插件模式 (MOD Launch)", ImVec2(btnWidth, btnHeight))) 
        {
             if (status.exeExists && status.dllExists) ModLoader::LaunchWithMod();
        }
        
        if (!status.exeExists || !status.dllExists) ImGui::PopStyleVar();
        ...
        if (ImGui::Button("配置管理 (Config)", ImVec2(btnWidth, btnHeight)))
        {
        ...
        ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.2f, 0.2f, 0.22f, 1.0f)); 
        ImGui::PushStyleVar(ImGuiStyleVar_FrameBorderSize, 1.0f);
        if (ImGui::Button("创建快捷方式 (Shortcut)", ImVec2(btnWidth, btnHeight)))
        {
        ...
        if (ImGui::BeginPopup("CreateShortcutPopup"))
        {
            ...
            if (ver == 105) { nameOriginal = "刀剑正传"; nameMod = "刀剑正传-MOD"; }
            else if (ver == 201) { nameOriginal = "上古传说"; nameMod = "上古传说-MOD"; }
    ...
    void RenderSettingsPage()
    {
        ...
        if (ImGui::Button("<- 返回主页 (Home)", ImVec2(150 * g_dpiScale, 40 * g_dpiScale)))
        {
            g_currentView = Home;
            // Optionally save on exit? Or just rely on explicit save.
        }
        ImGui::SameLine();
        ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), "|  配置详细设置");
        ImGui::Separator();
    ...
    if (activeSubTab == sec) {                                                                              \
        if (type == TYPE_BOOL) {                                                                           \
            ...
            if ((strcmp("res_enabled", #name) == 0)) {                                                     \
                if (ImGui::Checkbox(desc, &v)) g_pk_config.name = v;                                       \
                if (v) {                                                                                   \
                    ...
                    if (currentRes == -1) {                                                                \
                        for (int k = 0; k < 11; k++) {                                                     \
                            int w, h; sscanf_s(items[k], "%dx%d", &w, &h);                                 \
                            if (w == g_pk_config.res_width && h == g_pk_config.res_height) currentRes = k; \
                        }                                                                                  \
                    }                                                                                      \
                    if (ImGui::Combo("分辨率预设", &currentRes, items, 11)) {                               \
                        int w, h; sscanf_s(items[currentRes], "%dx%d", &w, &h);                            \
                        g_pk_config.res_width = w; g_pk_config.res_height = h;                             \
                    }                                                                                      \
                }                                                                                          \
            } else {                                                                                       \
                if (ImGui::Checkbox(desc, &v)) g_pk_config.name = v;                                       \
            ...
            if (g_pk_config.name != k) g_pk_config.name = k;                                               \
            if (ImGui::IsItemHovered()) ImGui::SetTooltip("组合键生效：Ctrl + %s", KeyCodeToString(k).c_str()); \
        }                                                                                                  \
        ImGui::Separator();                                                                                \
    }
...
#undef X
        ...
        if (ImGui::Button("保存所有配置 (Save All)", ImVec2(200 * g_dpiScale, 50 * g_dpiScale)))
        {
            ConfigManager::Save();
